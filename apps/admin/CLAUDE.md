...
## Notas específicas de Admin (Middleware y Autenticación)

- Middleware actualizado: usa cookies estandarizadas `altamedica_token` (fallback `auth-token`/`adminToken`) y redirige `/login` al gateway central `web-app`.
- No almacenar tokens en localStorage; el api-server setea cookies HttpOnly/Secure.
