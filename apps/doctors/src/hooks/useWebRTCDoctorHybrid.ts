...
interface UseWebRTCDoctorHybridReturn extends MedicalWebRTCState {
  ...
  captureSnapshot: (includePatient?: boolean) => Promise<{ doctorSnapshot: string; patientSnapshot?: string }>;
...
export function useWebRTCDoctorHybrid(
  ...
  useEffect(() => {
    if (firebase.enabled) {
      try {
        ...
        console.warn('Firebase no disponible para WebRTC:', err);
      }
    }
  }, [firebase.enabled]);
  ...
  // Verificar alertas de calidad
  const checkQualityAlerts = useCallback((metrics: any) => {
  ...
  const startCall = useCallback(async (): Promise<void> => {
    try {
      ...
      setWebRTCState(prev => ({
        ...prev,
        isConnecting: false,
        technicalAlerts: [...prev.technicalAlerts, {
  ...
  const startRecording = useCallback(async (options: Partial<MedicalRecordingOptions> = {}): Promise<void> => {
  ...
  }, [webrtcState.localStream, webrtcState.remoteStream, webrtcState.consentObtained, medical, config]);
  ...
  const verifyEncryption = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.error('Error verificando cifrado:', err);
      return false;
    }
  }, []);
  ...
  const stopRecording = useCallback(async (): Promise<{ recordingId: string; duration: number; size: number }> => {
    if (mediaRecorder.current && webrtcState.isRecording) {
      mediaRecorder.current.stop();
