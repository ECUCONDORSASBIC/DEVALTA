...
interface UseTelemedicineDoctorHybridReturn {
  ...
  // Calidad y métricas
  updateQualityMetrics: (metrics: Partial<DoctorTelemedicineSession['qualityMetrics']>) => Promise<void>;
...
export function useTelemedicineDoctorHybrid(
  ...
  useEffect(() => {
    if (firebase.enabled) {
      try {
        ...
        console.warn('Firebase no disponible, usando solo Socket.IO:', err);
      }
    }
  }, [firebase.enabled]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      socketInstance.on('medical_message_received', (message: MedicalChatMessage) => {
        ...
        if (firestore.current) {
          addDoc(collection(firestore.current, collections.messages!), message)
            .catch(console.error);
        }
      });
      ...
      socketInstance.on('vital_signs_received', (vitalSigns: DoctorVitalSigns) => {
        ...
        // Persistir en Firebase
        if (firestore.current) {
          addDoc(collection(firestore.current, collections.vitalSigns!), vitalSigns)
            .catch(console.error);
        }
      });
      ...
      socketInstance.on('medical_alert', (alert: any) => {
        ...
        console.warn('Alerta médica recibida:', alert);
  ...
  // Funciones principales
  const createSession = useCallback(async (sessionData: Partial<DoctorTelemedicineSession>): Promise<string> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (firestore.current) {
        const docRef = await addDoc(collection(firestore.current, collections.sessions!), newSession);
  ...
  const joinSession = useCallback(async (sessionId: string): Promise<void> => {
    ...
    try {
      ...
      if (firestore.current) {
        ...
        const vitalSigns = vitalSignsSnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        } as DoctorVitalSigns));
  ...
  const sendMedicalNote = useCallback(async (note: string, category: string, urgency: string): Promise<void> => {
  ...
  const recordVitalSigns = useCallback(async (vitalSignsData: Partial<DoctorVitalSigns>): Promise<void> => {
    if (!session) return;
  ...
  const addPrescription = useCallback(async (prescriptionData: Omit<MedicalPrescription, 'id'>): Promise<void> => {
    if (!session) return;
    ...
    await logMedicalAction('prescription_added', { prescriptionId: prescription.id, medication: prescription.medicationName });
  }, [session, socket, config.doctorId]);

  const endSession = useCallback(async (sessionId: string, sessionSummary: any): Promise<void> => {
    setIsLoading(true);
    ...
    try {
      ...
      const updatedSession = {
        ...
        endTime,
        duration: session ? Math.floor((new Date(endTime).getTime() - new Date(session.startTime).getTime()) / 1000) : 0,
        ...sessionSummary,
        updatedAt: endTime
      };
  ...
  const logMedicalAction = useCallback(async (action: string, details: any): Promise<void> => {
    ...
    if (firestore.current) {
      try {
        ...
        console.error('Error logging medical action:', err);
      }
    }
  ...
  const sendMessage = useCallback(async (message: string, messageType: MedicalChatMessage['messageType'] = 'text'): Promise<void> => {
    await sendMedicalNote(message, 'general', 'low');
  }, [sendMedicalNote]);

  const acknowledgeMessage = useCallback(async (messageId: string): Promise<void> => {
  ...
  const updatePatientStatus = useCallback(async (status: string, notes: string): Promise<void> => {
    if (!session) return;
  ...
  const updateDiagnosis = useCallback(async (diagnosis: string, confidence: string): Promise<void> => {
    if (!session) return;
  ...
  const addReferral = useCallback(async (referralData: Omit<MedicalReferral, 'id'>): Promise<void> => {
    if (!session) return;
  ...
  const transferSession = useCallback(async (newDoctorId: string, reason: string): Promise<void> => {
    if (socket && session) {
  ...
  const updateQualityMetrics = useCallback(async (metrics: Partial<DoctorTelemedicineSession['qualityMetrics']>): Promise<void> => {
    if (!session) return;
  ...
  const reportTechnicalIssue = useCallback(async (issue: string): Promise<void> => {
    if (!session) return;
