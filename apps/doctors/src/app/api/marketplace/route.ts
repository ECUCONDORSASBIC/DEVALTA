...
export const GET = requireRole(['doctor'], async (request: NextRequest, user: any) => {
  try {
    ...
    try {
      ...
      console.warn('Firestore unavailable, using fallback data:', firestoreError);
    ...
    console.error('Error obteniendo ofertas del marketplace:', error);
    return NextResponse.json(
      { 
        success: false, 
...
export const POST = requireRole(['doctor'], async (request: NextRequest, user: any) => {
  try {
    ...
    try {
      ...
      console.warn('Firestore unavailable, simulating application:', firestoreError);
