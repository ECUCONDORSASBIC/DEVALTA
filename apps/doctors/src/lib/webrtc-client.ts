...
export class WebRTCClient {
  ...
  async connect(): Promise<void> {
    try {
      ...
      console.log('Conectando al servidor WebRTC:', this.serverUrl);
  ...
  async publishStream(stream: MediaStream, type: 'video' | 'audio'): Promise<void> {
    try {
      ...
      console.log(`Publicando stream ${type}`);
  ...
  async setVideoQuality(quality: 'low' | 'medium' | 'high'): Promise<void> {
    try {
      console.log(`Configurando calidad de video: ${quality}`);
  ...
  async enableNoiseReduction(enabled: boolean): Promise<void> {
    try {
      console.log(`Supresión de ruido: ${enabled ? 'activada' : 'desactivada'}`);
  ...
  async setBandwidthOptimization(enabled: boolean): Promise<void> {
    try {
      console.log(`Optimización de ancho de banda: ${enabled ? 'activada' : 'desactivada'}`);
  ...
  disconnect(): void {
    try {
      console.log('Desconectando del servidor WebRTC');
