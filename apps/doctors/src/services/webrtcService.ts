...
class WebRTCService {
  ...
  private async connectSignaling(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.signalingSocket.onerror = (error) => {
        console.error('Error en conexión de señalización:', error);
        reject(error);
      };
  ...
  private handleUserJoined(message: SignalingMessage): void {
    console.log(`Usuario ${message.from} se unió a la sala`);
    this.emit('connection-state-change', { 
      state: 'user-joined', 
      userId: message.from 
    });
  ...
  private handleUserLeft(message: SignalingMessage): void {
    console.log(`Usuario ${message.from} dejó la sala`);
    this.emit('connection-state-change', { 
      state: 'user-left', 
      userId: message.from 
    });
  ...
  private sendSignalingMessage(message: SignalingMessage): void {
    if (this.signalingSocket && this.signalingSocket.readyState === WebSocket.OPEN) {
      ...
      console.warn('Socket de señalización no está conectado');
    }
  }
