...
class SocketManager {
  ...
  joinRoom(roomId: string, userId: string, userType: string): void {
    if (!this.socket || !this.isConnected) {
      console.error('❌ Socket no conectado para unirse a sala');
      return;
    }

    console.log(`🚪 Uniéndose a sala: ${roomId} como ${userType} (${userId})`);
  ...
  sendOffer(roomId: string, offer: RTCSessionDescriptionInit, targetUserId: string): void {
    if (!this.socket || !this.isConnected) {
      console.error('❌ Socket no conectado para enviar oferta');
      return;
    }

    console.log(`📤 Enviando oferta WebRTC a ${targetUserId}`);
  ...
  sendAnswer(roomId: string, answer: RTCSessionDescriptionInit, targetUserId: string): void {
    if (!this.socket || !this.isConnected) {
      console.error('❌ Socket no conectado para enviar respuesta');
      return;
    }

    console.log(`📤 Enviando respuesta WebRTC a ${targetUserId}`);
  ...
  sendIceCandidate(roomId: string, candidate: RTCIceCandidateInit, targetUserId: string): void {
    if (!this.socket || !this.isConnected) {
      console.error('❌ Socket no conectado para enviar candidato ICE');
      return;
    }

    console.log(`📤 Enviando candidato ICE a ${targetUserId}`);
  ...
  sendChatMessage(roomId: string, message: string, userId: string, userType: string): void {
    if (!this.socket || !this.isConnected) {
      console.error('❌ Socket no conectado para enviar mensaje');
      return;
    }
  ...
  on<T extends keyof SocketEvents>(event: T, callback: SocketEvents[T]): void {
    if (!this.socket) {
      console.error('❌ Socket no inicializado para escuchar eventos');
      return;
    }
