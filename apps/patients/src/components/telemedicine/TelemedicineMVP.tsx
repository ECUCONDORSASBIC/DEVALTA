...
export default function TelemedicineMVP({
  ...
  useEffect(() => {
    if (systemCheck.camera === 'success' && systemCheck.microphone === 'success' && systemCheck.connection === 'success') {
      if (consentGiven) {
        initializeSocket();
      } else {
        setPhase('waiting');
  ...
  const initializeWebRTC = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: 'Error WebRTC',
        description: 'No se pudo inicializar la videollamada',
        variant: 'destructive'
      });
    }
  }, [socket, roomId, toast]);
                ...
                <div className="p-4 bg-blue-50 rounded-lg">
                  ...
                  <label className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-blue-800">
                      Acepto realizar la consulta médica por videollamada y que la sesión sea grabada con fines médicos
                    </span>
                  </label>
                </div>
              </div>
