// 📋 HOOKS ESPECIALIZADOS MÉDICOS - ALTAMEDICA
// Gestión de registros médicos y prescripciones con estados optimizados
// CONSERVADOR: Funcionalidad granular, mantiene compatibilidad total

import { useState, useEffect, useCallback, useMemo } from 'react';
import { medicalService } from '../services';
import type { MedicalRecord, Prescription, PaginatedResponse, SymptomAnalysis, DrugInteractionCheck } from '../types';
import type { MedicalRecordFilters } from '../services/MedicalService';

// 📋 HOOK DE REGISTROS MÉDICOS
export function useMedicalRecords(patientId?: string, options: { initialFetch?: boolean; defaultLimit?: number } = {}) {
  ...
  const searchRecords = useCallback(async (filters: MedicalRecordFilters = {}) => {
    try {
      ...
      const response: PaginatedResponse<MedicalRecord> = await medicalService.getMedicalRecords(searchFilters);
...
export function usePrescriptions(patientId?: string, options: { initialFetch?: boolean } = {}) {
  ...
  const fetchPrescriptions = useCallback(async () => {
    try {
      ...
      const response: PaginatedResponse<Prescription> = await medicalService.getPrescriptions(patientId);
...
export function useMedicalDashboard(patientId?: string) {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      const recentRecords = recordsResponse.status === 'fulfilled' ? recordsResponse.value.data : [];
      const allPrescriptions = prescriptionsResponse.status === 'fulfilled' ? prescriptionsResponse.value.data : [];
