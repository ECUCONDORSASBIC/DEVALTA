...
export function useAppointments(options: UseAppointmentOptions = {}) {
  ...
  const searchAppointments = useCallback(async (filters?: any) => {
  ...
  const state: UseAppointmentState = {
    ...
    currentAppointment,
    loading: baseQuery.isLoading || createMutation.isPending || updateMutation.isPending || cancelMutation.isPending,
    error: baseQuery.error?.message || createMutation.error?.message || updateMutation.error?.message || cancelMutation.error?.message || null,
