...
export const useRealTimeUpdates = (patientId: string) => {
  const subscribe = useCallback((callbacks: UpdateCallbacks) => {
    ...
    console.log(`Suscripción activada para paciente ${patientId}`);
    ...
    return {
      ...
      unsubscribe: () => {
        ...
        console.log(`Suscripción cancelada para paciente ${patientId}`);
      }
    };
  }, [patientId]);

  const unsubscribe = useCallback((subscriptionId: string) => {
    console.log(`Desuscripción manual para ${subscriptionId}`);
  }, []);
