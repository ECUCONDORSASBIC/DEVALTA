 ...
 * Versión mejorada con diseño moderno y botones desplegables
 */
...
export default function HospitalNetworkDashboard({ hospitalId, config }: DashboardProps) {
  ...
  const [dataSource, setDataSource] = useState<'whatsapp' | 'api' | 'iot' | 'mixed'>('mixed');
  
  // 🔄 Patient Redistribution State
  ...
  // 👨‍⚕️ Staff Shortage State
  const [staffShortages, setStaffShortages] = useState<StaffShortage[]>([]);
  const [jobPostings, setJobPostings] = useState<JobPosting[]>([]);
  
  // 🎛️ System Control State
  const [autoRedistributionEnabled, setAutoRedistributionEnabled] = useState(true);
  const [autoJobPostingEnabled, setAutoJobPostingEnabled] = useState(true);
  ...
  const loadHospitalData = async () => {
    try {
      ...
      const defaultData: HospitalMetrics = {
        hospitalId: hospitalId,
        timestamp: new Date(),
        occupancy: {
          beds: {
  ...
  const handleRouteSelectOnMap = (route: any) => {
    console.log('Ruta seleccionada en mapa:', route);
  };
