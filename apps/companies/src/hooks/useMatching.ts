...
export function useRealTimeMatching(companyId: string) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      ...
      console.log('🔗 Conectado a matching en tiempo real')
    }
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === 'new_match') {
        ...
        console.log('🎯 Nuevo match disponible:', data)
    ...
    ws.onclose = () => {
      ...
      console.log('🔌 Desconectado del matching en tiempo real')
    }

    ws.onerror = (error) => {
      console.error('❌ Error en WebSocket matching:', error)
      setConnectionStatus('disconnected')
    }
