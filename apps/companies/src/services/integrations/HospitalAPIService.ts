...
export class HospitalAPIClient {
  ...
  async fetchHospitalStatus(hospitalId: string): Promise<HospitalMetrics | null> {
    if (!this.config.enabled) {
      console.log('🔌 Hospital API disabled, returning mock data');
    ...
    try {
      ...
      console.error(`❌ Failed to fetch hospital status for ${hospitalId}:`, error);
  ...
  async saveHistoricalData(hospitalId: string, data: HospitalMetrics): Promise<void> {
    if (!this.config.enabled) {
      console.log('🔌 Hospital API disabled, skipping historical save');
      return;
    }
    ...
    try {
      ...
      console.error(`❌ Failed to save historical data for ${hospitalId}:`, error);
    }
  }
  ...
  private async notifyNetworkEmergency(hospitalId: string, emergency: any): Promise<void> {
    try {
      ...
      console.error('Failed to notify network emergency:', error);
    }
  }
