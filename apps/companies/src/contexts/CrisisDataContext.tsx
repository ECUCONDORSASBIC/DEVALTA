...
const DEFAULT_SEEDS: CrisisHospitalSeed[] = [
  { id: 'h1', name: 'Hospital Central', city: 'Centro', country: 'AR', coordinates: [-34.6037, -58.3816] },
  { id: 'h2', name: 'Clínica Norte', city: 'Zona Norte', country: 'AR', coordinates: [-34.55, -58.45] },
  { id: 'h3', name: 'Hospital Sur', city: 'Zona Sur', country: 'AR', coordinates: [-34.7, -58.4] },
  { id: 'h4', name: 'Centro Médico Este', city: 'Zona Este', country: 'AR', coordinates: [-34.6, -58.25] },
  { id: 'h5', name: 'Hospital Universitario', city: 'Universidad', country: 'AR', coordinates: [-34.6, -58.5] },
  { id: 'h6', name: 'Clínica Oeste', city: 'Zona Oeste', country: 'AR', coordinates: [-34.62, -58.6] },
];

export function CrisisDataProvider({ children, hospitalSeeds = DEFAULT_SEEDS, refreshIntervalMs = 30000, useMock = true }: CrisisDataProviderProps) {
  ...
  const [networkStats, setNetworkStats] = useState<CrisisNetworkStats>({ activeUsers: 0, alerts: 0, systemLoad: 0, networkConnected: true });
  ...
  const toMapHospital = useCallback((seed: CrisisHospitalSeed, metrics: HospitalMetrics): MapHospital => {
    const percentage = metrics.occupancy.beds.percentage || 0;
    const status: MapHospital['status'] = percentage > 95 ? 'saturated' : percentage > 85 ? 'critical' : percentage > 75 ? 'warning' : 'normal';
    return {
      id: seed.id,
      name: seed.name,
      location: {
  ...
  const buildMockMetrics = useCallback((seed: CrisisHospitalSeed): HospitalMetrics => {
    ...
    return {
      ...
      timestamp: new Date(),
      occupancy: {
        beds: { total: totalBeds, occupied, available: Math.max(0, totalBeds - occupied), percentage: Math.min(100, occ) },
  ...
  const computeRoutes = useCallback((h: MapHospital[]): RedistributionRoute[] => {
    ...
    overs.forEach((from, idx) => {
      ...
      out.push({
        ...
        fromHospital: from,
        toHospital: to,
        patientsToTransfer: Math.max(3, Math.round((from.currentCapacity - from.maxCapacity * 0.85) * 0.2)),
  ...
  const computeShortages = useCallback((h: MapHospital[]): StaffShortage[] => {
    ...
    h.forEach((hos) => {
      ...
      if (needNurses > 0) shortages.push({ hospitalId: hos.id, hospitalName: hos.name, role: 'Enfermería', shortage: needNurses, severity: needNurses > 10 ? 'critical' : 'urgent', autoJobPostingTriggered: needNurses > 5 });
      if (needDocs > 0) shortages.push({ hospitalId: hos.id, hospitalName: hos.name, role: 'Médicos', shortage: needDocs, severity: needDocs > 5 ? 'urgent' : 'moderate', autoJobPostingTriggered: needDocs > 3 });
    });
    return shortages;
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setNetworkStats({
        ...
        alerts: Math.round(mapped.filter(h => h.status === 'critical' || h.status === 'saturated').length * 2 + Math.random() * 5),
  ...
  return (
    <CrisisDataContext.Provider value={value}>{children}</CrisisDataContext.Provider>
  );
}
