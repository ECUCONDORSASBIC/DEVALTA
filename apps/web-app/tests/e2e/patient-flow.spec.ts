...
async function waitForLoadingToDisappear(page: Page) {
  ...
  try {
    ...
    console.log('⚠️ Timeout esperando que desaparezcan los loaders, continuando...');
  }
}
...
test.describe('🏥 Flujo Completo de Paciente - AltaMedica', () => {
  ...
  test('1️⃣ Registro de nuevo paciente', async ({ page }) => {
    console.log('🧪 Iniciando test de registro con:', testPatient.email);
  ...
  test('3️⃣ Navegación y acciones del paciente', async ({ page }) => {
    console.log('🧪 Iniciando test de navegación del paciente');
  ...
  test('4️⃣ Funcionalidades específicas del paciente', async ({ page }) => {
    console.log('🧪 Testing funcionalidades específicas');
  ...
  test('5️⃣ Verificar accesibilidad y diseño responsive', async ({ page }) => {
    console.log('🧪 Testing accesibilidad y responsive');
...
// 🧹 CLEANUP
test.afterAll(async () => {
  console.log('✅ Tests E2E de paciente completados');
});