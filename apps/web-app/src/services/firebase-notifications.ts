...
export interface MedicalNotification {
  ...
  senderId?: string
  senderName?: string
  type: 'message' | 'appointment' | 'prescription' | 'lab_result' | 'reminder' | 'emergency' | 'system'
  title: string
  body: string
  data?: {
...
export interface NotificationService {
  ...
  sendNotification: (recipientId: string, notification: Omit<MedicalNotification, 'id' | 'recipientId' | 'createdAt' | 'read'>) => Promise<void>
  sendBulkNotifications: (recipientIds: string[], notification: Omit<MedicalNotification, 'id' | 'recipientId' | 'createdAt' | 'read'>) => Promise<void>
  ...
  subscribeToNotifications: (userId: string, callback: (notifications: MedicalNotification[]) => void) => () => void
...
class FirebaseNotificationService implements NotificationService {
  ...
  subscribeToNotifications(
    ...
    try {
      ...
      return onSnapshot(q, (querySnapshot) => {
        const notifications: MedicalNotification[] = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        } as MedicalNotification))
  ...
  async getNotificationPreferences(userId: string): Promise<NotificationPreferences | null> {
    try {
      const preferencesRef = doc(db, this.userPreferencesCollection, userId)
      const preferencesSnap = await getDocs(query(collection(db, this.userPreferencesCollection), where('__name__', '==', userId)))
  ...
  private async sendPushNotification(
    ...
    try {
      ...
      console.log('Enviando push notification urgente a:', recipientId, notification.title)
