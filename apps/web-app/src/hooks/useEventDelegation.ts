...
export function useEventDelegation(config: EventDelegationConfig = {}) {
  ...
  const handleMouseEvent = useCallback((event: MouseEvent) => {
    ...
    if (debug) {
      console.log('Mouse event:', event.type, 'Target:', target)
    }

    if (shouldBlockEvent(target)) {
      if (debug) {
        console.log('Blocking mouse event for UI element')
      }
    ...
    if (shouldPassThrough(target) && enableMousePassthrough) {
      if (debug) {
        console.log('Passing through mouse event to scene')
      }
  ...
  const handleTouchEvent = useCallback((event: TouchEvent) => {
    ...
    if (debug) {
      console.log('Touch event:', event.type, 'Target:', target)
    }

    if (shouldBlockEvent(target)) {
      if (debug) {
        console.log('Blocking touch event for UI element')
      }
      onUIInteraction?.(event)
      return
    }
    ...
    if (shouldPassThrough(target) && enableTouchPassthrough) {
      if (debug) {
        console.log('Passing through touch event to scene')
      }
      onSceneInteraction?.(event)
