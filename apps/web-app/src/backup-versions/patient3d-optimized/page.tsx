          ...
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {animationList.map((animName, index) => (
              <button
                ...
                className={`w-full text-left px-2 py-1 text-xs rounded transition-colors ${currentAnimation === animName ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                {animName}
              </button>
            ))}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-green-900 to-blue-900">
      <div className="text-center text-white">
        ...
        <p className="text-lg mb-2">Preparando modelo optimizado...</p>
        <p className="text-sm text-green-200">Versión optimizada para mejor rendimiento</p>
      </div>
    </div>
  )
}

export default function OptimizedPatient3DPage() {
