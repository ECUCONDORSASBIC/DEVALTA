// components/anamnesis/ResumenAnamnesis.tsx
import React, { useState, useEffect } from 'react'
import { motion } from 'framer-motion'
import { RespuestaAnamnesis, Logro } from '../../types/anamnesis.types'
import { generarResumenMedico, analizarRespuestasParaRecomendaciones } from '../../services/anamnesisService'
import confetti from 'canvas-confetti'

interface ResumenAnamnesisProps {
...
export function ResumenAnamnesis(props: ResumenAnamnesisProps) {
  ...
  const descargarResumen = () => {
    const blob = new Blob([resumenMedico], { type: 'text/markdown' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `anamnesis_${new Date().toISOString().split('T')[0]}.md`
    a.click()
    URL.revokeObjectURL(url)
    
    if (props.onDescargar) props.onDescargar()
  }

  return (
    <motion.div
      className="fixed inset-0 z-50 bg-gradient-to-br from-purple-100 to-indigo-100 overflow-auto"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    >
      <div className="min-h-screen p-8">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <motion.h1
              className="text-5xl font-bold text-center mb-6 bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent"
            ...
            <p className="text-xl text-gray-700">
              Has completado exitosamente tu historia médica interactiva
            </p>
          </motion.div>
          ...
          >
            ...
            {mostrandoResumen && (
              <motion.div
                className="bg-gray-50 p-6 rounded-lg font-mono text-sm overflow-auto max-h-96"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 1 }}
              >
            ...
            <div className="flex gap-4 mt-6">
              <motion.button
                ...
                className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all"
              ...
              {props.onCerrar && (
                <motion.button
                  ...
                  className="px-6 py-3 bg-gray-500 text-white rounded-lg font-semibold shadow-lg hover:shadow-xl transition-all"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Cerrar
