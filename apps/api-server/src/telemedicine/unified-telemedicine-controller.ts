...
export class WebRTCServer {
  ...
  private setupSocketHandlers() {
    ...
    simpleNs.on('connection', (socket) => {
      console.log(`[simple] client connected: ${socket.id}`);
      ...
      socket.on('join-room', (data: { roomId: string; userId?: string; userType?: string }) => {
        try {
          ...
          console.log(`[simple] ${socket.id} joined room ${roomId} (${userType || 'unknown'})`);
        } catch (err) {
          console.error('[simple] join-room error:', err);
      ...
      socket.on('disconnect', () => {
        console.log(`[simple] client disconnected: ${socket.id}`);
      });
    });
  }
}
...
export class TelemedicineService {
  ...
  constructor() {
    ...
    if (!this.useFirebase) {
      console.warn('Firebase not available. Using Prisma database.');
    }
  }
