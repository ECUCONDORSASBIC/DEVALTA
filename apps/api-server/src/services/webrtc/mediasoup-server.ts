...
export class MediaSoupServer {
  ...
  private async initialize() {
    try {
      ...
      console.log('✅ MediaSoup Server inicializado correctamente');
    } catch (error) {
      console.error('❌ Error al inicializar MediaSoup Server:', error);
    }
  }
  ...
  private async createWorkers() {
    ...
    for (let i = 0; i < numWorkers; i++) {
      ...
      console.log(`✅ Worker ${i + 1} creado (PID: ${worker.pid})`);
    }
  }
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('create-producer-transport', async (data: { roomId: string }) => {
        try {
          ...
          console.error('Error en create-producer-transport:', error);
      ...
      socket.on('connect-producer-transport', async (data: { transportId: string; dtlsParameters: mediasoup.types.DtlsParameters }) => {
        try {
          ...
          console.error('Error en connect-producer-transport:', error);
      ...
      socket.on('create-consumer-transport', async (data: { roomId: string }) => {
        try {
          ...
          console.error('Error en create-consumer-transport:', error);
      ...
      socket.on('connect-consumer-transport', async (data: { transportId: string; dtlsParameters: mediasoup.types.DtlsParameters }) => {
        try {
          ...
          console.error('Error en connect-consumer-transport:', error);
      ...
      socket.on('pause-producer', async (data: { producerId: string }) => {
        try {
          ...
          console.error('Error en pause-producer:', error);
        }
      });
      ...
      socket.on('resume-producer', async (data: { producerId: string }) => {
        try {
          ...
          console.error('Error en resume-producer:', error);
        }
      });
      ...
      socket.on('close-transport', async (data: { transportId: string }) => {
        try {
          ...
          console.error('Error en close-transport:', error);
        }
      });
  ...
  private async handleJoinRoom(socket: any, data: { roomId: string; rtpCapabilities?: mediasoup.types.RtpCapabilities }) {
    ...
    console.log(`👤 Peer ${socket.id} se unió a la sala ${roomId}`);
  }
  ...
  private async handleCreateProducerTransport(socket: any, data: { roomId: string }) {
    ...
    transport.on('dtlsstatechange', (dtlsState) => {
      console.log(`🔐 Transport DTLS state: ${dtlsState}`);
    });

    transport.on('close', () => {
      console.log(`🚪 Transport cerrado: ${transport.id}`);
    });
  ...
  private async handleProduce(socket: any, data: { transportId: string; kind: mediasoup.types.MediaKind; rtpParameters: mediasoup.types.RtpParameters; appData?: any }) {
    ...
    producer.on('transportclose', () => {
      console.log(`🚪 Producer transport cerrado: ${producer.id}`);
    });

    producer.on('close', () => {
      console.log(`🚪 Producer cerrado: ${producer.id}`);
  ...
  private async handleCreateConsumerTransport(socket: any, data: { roomId: string }) {
    ...
    transport.on('dtlsstatechange', (dtlsState) => {
      console.log(`🔐 Consumer transport DTLS state: ${dtlsState}`);
    });

    transport.on('close', () => {
      console.log(`🚪 Consumer transport cerrado: ${transport.id}`);
    });
  ...
  private async handleConsume(socket: any, data: { transportId: string; producerId: string; rtpCapabilities: mediasoup.types.RtpCapabilities }) {
    ...
    consumer.on('transportclose', () => {
      console.log(`🚪 Consumer transport cerrado: ${consumer.id}`);
    });

    consumer.on('close', () => {
      console.log(`🚪 Consumer cerrado: ${consumer.id}`);
    ...
    for (const room of this.rooms.values()) {
      ...
      if (peer) {
        ...
        if (room.peers.size === 0) {
          ...
          console.log(`🏠 Sala cerrada: ${room.id}`);
        }
  ...
  public closeRoom(roomId: string) {
    ...
    if (room) {
      ...
      console.log(`🏠 Sala cerrada manualmente: ${roomId}`);
    }
  }
} 