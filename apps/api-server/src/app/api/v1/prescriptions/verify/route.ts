...
export async function GET(request: NextRequest) {
  try {
    ...
    if ((verifyData as any).patientId && (prescriptionData as any).patientId !== (verifyData as any).patientId) {
      return NextResponse.json(
    ...
    if (verifyData.checkDigitalSignature) {
      ...
      const expectedSignature = `DR_${(prescriptionData as any).doctorId}_${prescriptionData.prescriptionNumber.split('-')[1]}`;
    ...
    try {
      ...
      dispensingHistory = dispensingSnapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
    ...
    const doctorProfileDoc = await adminDb.collection('doctors').doc((prescriptionData as any).doctorId).get();
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Parámetros de verificación inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos de dispensación inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
