...
export async function GET(request: NextRequest) {
  try {
    ...
    const prescriptions = prescriptionsSnapshot.docs.map((doc: any) => ({
      id: doc.id,
      ...doc.data(),
    ...
    if (queryData.role === 'doctor') {
      ...
      const completedAppointments = appointments.filter((apt: any) => (apt as any).status === 'completed');
      ...
      const activePrescriptions = prescriptions.filter((pres: any) => {
        const validUntil = (pres as any).validUntil?.toDate?.() ?? new Date((pres as any).validUntil);
        return validUntil > now && (pres as any).status !== 'cancelled';
      });
      ...
      dashboardData = {
        overview: {
          ...
          completedAppointments: appointments.filter((apt: any) => (apt as any).status === 'completed').length,
        },
      ...
      const verifiedDoctors = doctorsSnapshot.docs.filter((doc: any) => doc.data().isVerified).length;
