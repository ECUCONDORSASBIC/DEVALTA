...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if ((updateData as any).scheduledAt) {
      const appointmentData = appointmentDoc.data()!;
      ...
      const conflictQuery = adminDb
        ...
        .where('scheduledAt', '>=', (updateData as any).scheduledAt)
        .where('scheduledAt', '<', new Date((updateData as any).scheduledAt.getTime() + duration * 60000));

      const conflictSnapshot = await conflictQuery.get();
      
      // Filtrar el documento actual
      const conflicts = conflictSnapshot.docs.filter((doc: any) => doc.id !== id);
