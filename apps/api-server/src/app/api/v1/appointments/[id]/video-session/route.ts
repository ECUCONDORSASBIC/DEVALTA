...
export async function GET(
  ...
  try {
    ...
    const videoSession = {
      ...
      appointmentId,
      status: isExpired ? 'expired' : (sessionData as any).status,
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      createSuccessResponse({
        ...
        participants: {
          doctor: {
            id: (appointmentData as any).doctorId,
            joinUrl: (joinUrls as any).doctor,
          },
          patient: {
            id: (appointmentData as any).patientId,
            joinUrl: (joinUrls as any).patient,
          },
        },
      }),
      { status: 201 }
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Configuración de sesión inválida', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
...
export async function PUT(
  ...
  try {
    ...
    if (updateData.metadata) {
      Object.keys(updateData.metadata).forEach((key: any) => {
