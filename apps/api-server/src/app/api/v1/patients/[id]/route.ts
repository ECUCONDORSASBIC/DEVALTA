...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const appointmentsSnapshot = await adminDb
      ...
      .where('patientId', '==', id)
      .get();    const appointments = appointmentsSnapshot.docs.map((doc: any) => doc.data());
    const appointmentStats = {
      total: appointments.length,
      completed: appointments.filter((apt: AppointmentData) => (apt as any).status === 'completed').length,
      cancelled: appointments.filter((apt: AppointmentData) => (apt as any).status === 'cancelled').length,
      upcoming: appointments.filter((apt: AppointmentData) => 
        ['scheduled', 'confirmed'].includes((apt as any).status) && 
        new Date((apt as any).scheduledAt) > new Date()
      ).length,
    };
    ...
    const upcomingAppointments = appointments
      ...
      .sort((a: AppointmentData, b: AppointmentData) => new Date((a as any).scheduledAt).getTime() - new Date((b as any).scheduledAt).getTime());

    const nextAppointment = upcomingAppointments[0] || null;

    // Obtener última cita
    const pastAppointments = appointments
      .filter((apt: AppointmentData) => new Date((apt as any).scheduledAt) <= new Date())
      .sort((a: AppointmentData, b: AppointmentData) => new Date((b as any).scheduledAt).getTime() - new Date((a as any).scheduledAt).getTime());
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos de actualización inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
