...
export async function GET(request: NextRequest) {
  const startTime = Date.now();
    ...
    diagnostics.tests.push(await testIndexes());
...
async function testFirestoreConnection() {
  ...
  try {
    ...
    // Intentar obtener un documento de la colección de health checks
    const healthRef = adminDb.collection('health-checks').doc('test');
    await healthRef.set({
      timestamp: new Date().toISOString(),
...
async function testIndexes() {
  ...
  try {
    // Test query que requiere índice compuesto
    const testStart = Date.now();
    const queryResult = await adminDb
    ...
    test.details = {
      ...
      documentsFound: queryResult.size,
      indexStatus: queryTime < 1000 ? 'optimized' : 'slow',
    };

    if (queryTime > 1000) {
      (test.recommendations as any[]).push('Crear índices compuestos para mejorar rendimiento');
      (test.recommendations as any[]).push('Revisar archivo firestore.indexes.json');
    }

  } catch (error: unknown) {
    if ((error as any).code === 9) { // FAILED_PRECONDITION - missing index
      test.status = 'fail';
      test.details = {
        error: 'Missing composite index',
        code: 'MISSING_INDEX',
      };
      (test.recommendations as any[]).push('Crear índices requeridos en Firebase Console');
      (test.recommendations as any[]).push('Desplegar firestore.indexes.json');
    } else {
      test.status = 'fail';
      test.details = {
        error: (error as any).message,
