...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter by recipient
    query = (query as any).where('recipient_id', 'in', [authResult.user.id, 'all']);

    // Filter by read status
    if (unread_only) {
      query = (query as any).where('is_read', '==', false);
    }
    ...
    query = (query as any).where('expires_at', '>', now);

    // Sort by creation date (newest first)
    query = (query as any).orderBy('created_at', 'desc');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validTypes.includes((body as any).type)) {
      return NextResponse.json({
        success: false,
