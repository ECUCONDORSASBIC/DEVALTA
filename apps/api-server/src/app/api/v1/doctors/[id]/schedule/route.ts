...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    while (currentDate <= endDate) {
          ...
          while (slotStart < slotEnd) {
            ...
            if (appointment && queryData.includeBooked) {
              ...
              if ((appointment as any).patientId) {
                try {
                  const patientDoc = await adminDb.collection('users').doc((appointment as any).patientId).get();
      ...
      const bookedSlots = timeSlots.filter((slot: any) => !slot.isAvailable).length;
    ...
    return NextResponse.json(
      createSuccessResponse({
        ...
        summary: {
          ...
          workingDays: schedule.filter((day: any) => day.isWorkingDay).length,
