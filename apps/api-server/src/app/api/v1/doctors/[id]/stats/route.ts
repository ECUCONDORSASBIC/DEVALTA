...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const completedAppointments = appointments.filter((apt: any) => (apt as any).status === 'completed').length;
    const cancelledAppointments = appointments.filter((apt: any) => (apt as any).status === 'cancelled').length;
    const noShowAppointments = appointments.filter((apt: any) => (apt as any).status === 'no-show').length;
    const scheduledAppointments = appointments.filter((apt: any) => 
      ['scheduled', 'confirmed', 'in-progress'].includes((apt as any).status)
    ).length;
    ...
    const appointmentsByType = appointments.reduce((acc, apt) => {
      acc[(apt as any).type] = (acc[(apt as any).type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    // Pacientes únicos atendidos
    const uniquePatients = [...new Set(appointments.map((apt: any) => (apt as any).patientId))];
    ...
    for (const patientId of uniquePatients) {
      ...
      if (!allPatientAppointments.empty) {
        ...
        const firstAppointmentDate = (firstAppointment as any).scheduledAt?.toDate?.() || (firstAppointment as any).scheduledAt;
    ...
    while (currentDate <= endDate) {
      ...
      const dayAppointments = appointments.filter((apt: any) => {
        const aptDate = new Date((apt as any).scheduledAt);
      ...
      dailyStats[dateKey] = {
        ...
        completed: dayAppointments.filter((apt: any) => (apt as any).status === 'completed').length,
        cancelled: dayAppointments.filter((apt: any) => (apt as any).status === 'cancelled').length,
        scheduled: dayAppointments.filter((apt: any) => 
          ['scheduled', 'confirmed'].includes((apt as any).status)
        ).length,
        revenue: dayAppointments.filter((apt: any) => (apt as any).status === 'completed').length * consultationFee,
      };
    ...
    appointments.forEach((apt: any) => {
      const hour = new Date((apt as any).scheduledAt).getHours();
