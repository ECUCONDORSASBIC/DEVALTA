...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (queryData.date) {
      ...
      const bookedSlots = existingAppointments.docs.map((doc: any) => {
        const appointment = doc.data();
        const scheduledAt = (appointment as any).scheduledAt.toDate();
    ...
    if (queryData.startDate && queryData.endDate) {
      ...
      return NextResponse.json(
        createSuccessResponse({
          ...
          daysWithAvailability: dateRange.filter((d: any) => d.hasAvailability).length,
          dateRange,
        }),
        { status: 200 }
      );
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos de disponibilidad inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
