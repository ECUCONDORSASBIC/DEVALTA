...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    query = (query as any).orderBy('scheduledAt', 'asc');
    ...
    for (const doc of snapshot.docs) {
      ...
      if ((appointmentData as any).patientId) {
        try {
          ...
          if (patientDoc.exists) {
            ...
            const patientProfileDoc = await adminDb.collection('patients').doc((appointmentData as any).patientId).get();
          ...
          console.error(`Error fetching patient data for ${(appointmentData as any).patientId}:`, error);
        }
      }
    ...
    const stats = {
      ...
      upcomingToday: appointments.filter((apt: any) => {
