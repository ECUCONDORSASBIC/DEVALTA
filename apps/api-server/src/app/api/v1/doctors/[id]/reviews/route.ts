...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (queryData.rating) {
      query = (query as any).where('rating', '==', queryData.rating);
    }
    ...
    switch (queryData.sortBy) {
      case 'newest':
        ...
        break;
      case 'oldest':
        query = (query as any).orderBy('createdAt', 'asc');
        break;
      case 'rating_high':
        query = (query as any).orderBy('rating', 'desc').orderBy('createdAt', 'desc');
        break;
      case 'rating_low':
        query = (query as any).orderBy('rating', 'asc').orderBy('createdAt', 'desc');
        break;
    }
    ...
    for (const doc of snapshot.docs) {
      ...
      if ((reviewData as any).patientId) {
        try {
          const patientDoc = await adminDb.collection('users').doc((reviewData as any).patientId).get();
          if (patientDoc.exists) {
            const patientUserData = patientDoc.data();
            patientInfo = {
              id: (reviewData as any).patientId,
      ...
      reviews.push({
        id: doc.id,
        ...reviewData,
        createdAt: (reviewData as any).createdAt?.toDate?.() ?? (reviewData as any).createdAt,
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos de reseña inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
