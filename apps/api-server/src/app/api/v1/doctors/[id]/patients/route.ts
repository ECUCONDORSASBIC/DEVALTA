  ...
  try {
    ...
    for (const patientId of patientIds) {
      try {
        ...
        const completedAppointments = patientAppointments.filter((apt: any) => (apt as any).status === 'completed').length;
        const cancelledAppointments = patientAppointments.filter((apt: any) => (apt as any).status === 'cancelled').length;
        ...
        const upcomingAppointments = patientAppointments
          ...
          .sort((a, b) => new Date((a as any).scheduledAt).getTime() - new Date((b as any).scheduledAt).getTime());

        const pastAppointments = patientAppointments
          .filter((apt: any) => new Date((apt as any).scheduledAt) <= now)
          .sort((a, b) => new Date((b as any).scheduledAt).getTime() - new Date((a as any).scheduledAt).getTime());
        ...
        const patient = {
          ...
          firstAppointment: patientAppointments
            .sort((a, b) => new Date((a as any).scheduledAt).getTime() - new Date((b as any).scheduledAt).getTime())[0] || null,
        };
    ...
    patients.sort((a, b) => {
      ...
      switch (queryData.sortBy) {
        ...
        case 'lastAppointment':
          const lastA = a.lastAppointment ? new Date((a as any).scheduledAt).getTime() : 0;
          const lastB = b.lastAppointment ? new Date((b as any).scheduledAt).getTime() : 0;
          comparison = lastA - lastB;
          break;
        case 'nextAppointment':
          const nextA = a.nextAppointment ? new Date((a as any).scheduledAt).getTime() : Infinity;
          const nextB = b.nextAppointment ? new Date((b as any).scheduledAt).getTime() : Infinity;
