...
export async function GET(request: NextRequest) {
  try {
    ...
    let query: any = adminDb.collection('doctors');

    // Aplicar filtros
    if (searchData.specialty) {
      query = (query as any).where('specialties', 'array-contains', searchData.specialty);
    }

    if (searchData.companyId) {
      query = (query as any).where('companyId', '==', searchData.companyId);
    }

    if (searchData.isVerified !== undefined) {
      query = (query as any).where('isVerified', '==', searchData.isVerified);
    }

    // Obtener total de documentos para paginación (sin ordenamiento para evitar error de índice)
    const totalSnapshot = await query.get();
    const total = totalSnapshot.size;

    // Aplicar paginación y ordenamiento en memoria
    ...
    for (const doc of snapshot.docs) {
      ...
      if (userData) {
        doctors.push({
          ...
          createdAt: (doctorData as any).createdAt,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos del doctor inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
