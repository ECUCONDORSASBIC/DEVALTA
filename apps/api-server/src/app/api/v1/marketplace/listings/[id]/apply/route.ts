...
export const POST = createAuthenticatedRoute(
  async (request: NextRequest, authContext, { params }: { params: { id: string } }) => {
    try {
      ...
      console.log(`[Marketplace] Applying to listing ${id}`);
      ...
      console.log(`[Marketplace] Application created: ${application.id}`);
      ...
      console.error('[Marketplace] Error applying to listing:', error);
