...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const completedAppointments = appointments.filter((apt: {status: string}) => (apt as any).status === 'completed').length;

    const company = {
      id: companyDoc.id,
      ...companyData,
      createdAt: (companyData as any).createdAt?.toDate?.() ?? (companyData as any).createdAt,
      ...
      stats: {
        ...
        appointments: {
          ...
          cancelled: appointments.filter((apt: any) => (apt as any).status === 'cancelled').length,
        },
      },
      ...
      recentDoctors: doctors
        .sort((a, b) => {
          const aDate = (a as any).createdAt?.toDate?.() ?? new Date((a as any).createdAt);
          const bDate = (b as any).createdAt?.toDate?.() ?? new Date((b as any).createdAt);
