...
export async function GET(request: NextRequest) {
  try {
    ...
    let query: any = adminDb.collection('companies');

    // Aplicar filtros
    if ((searchData as any).type) {
      query = (query as any).where('type', '==', (searchData as any).type);
    }
    ...
    let companies = totalSnapshot.docs.map((doc: any) => ({
      id: doc.id,
      ...doc.data(),
      ...
      paginatedCompanies.map(async (company: any) => {
        ...
        return {
          ...
          stats: {
            ...
            activeJobsCount: jobsSnapshot.docs.filter((doc: any) => doc.data().status === 'active').length,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        createErrorResponse('VALIDATION_ERROR', 'Datos de empresa inválidos', { validationErrors: error.errors }),
        { status: 400 }
      );
    }
