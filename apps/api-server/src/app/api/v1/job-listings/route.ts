...
export async function GET(request: NextRequest) {
  try {
    ...
    // Apply filters
    if (specialty) {
      query = (query as any).where('position.specialty', '==', specialty);
    }
    
    if (type) {
      query = (query as any).where('(position as any).type', '==', type);
    }
    
    if (experience) {
      query = (query as any).where('position.experience_level', '==', experience);
    }
    
    if (remote) {
      query = (query as any).where('location.remote', '==', true);
    }
    
    if (company_id) {
      query = (query as any).where('company.id', '==', company_id);
    }
    
    if (status) {
      query = (query as any).where('status', '==', status);
    }    // Apply sorting
    query = (query as any).orderBy(sort, order);

    // Apply pagination
    const offset = (page - 1) * limit;
    query = (query as any).offset(offset).limit(limit);

    const snapshot = await query.get();    const job_listings = snapshot.docs.map((doc: any) => ({
      id: doc.id,
      ...doc.data()
    })) as JobListing[];
    ...
    if (location) {
      ...
      filteredListings = job_listings.filter((job: any) => 
...
export async function POST(request: NextRequest) {
  try {
    ...
    const jobListing: Omit<JobListing, 'id'> = {
      ...
      position: {
        type: (body as any).type || 'full_time',
