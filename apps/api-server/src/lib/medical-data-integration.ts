...
export class HealthcareIntegrationService {
  ...
  static async fetchLabResults(patientId: string, labSystem: string): Promise<any[]> {
    ...
    try {
      ...
      console.error(`Lab integration error for ${labSystem}:`, error);
  ...
  static async syncWithHIS(patientId: string): Promise<void> {
    try {
      ...
      if (!hisEndpoint || !apiKey) {
        console.warn('HIS integration not configured');
        return;
      }
      // Log del acceso
      await MedicalDataManager.logDataAccess(
      ...
      // - Resolver conflictos
      console.log(`HIS sync completed for patient ${patientId}`);
    } catch (error) {
  ...
  return function(
    ...
    return async function(request: NextRequest): Promise<NextResponse> {
      try {
        ...
        console.error('Medical compliance middleware error:', error);
        return Response.json(
