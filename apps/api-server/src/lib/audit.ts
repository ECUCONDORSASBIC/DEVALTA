...
export async function auditLog(entry: {
  ...
  try {
    ...
    if (!validation.success) {
      console.error('Invalid audit log entry:', validation.error);
      return;
    }
    ...
    const auditEntry: AuditLogEntry = {
      ...
      severity: validEntry.severity || severity,
      category: validEntry.category || category,
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[AUDIT] ${auditEntry.action} by ${auditEntry.userEmail} on ${auditEntry.resource}`, {
    ...
    await emergencyAuditLog(entry, error);
  }
}
...
async function writeToDestinations(entry: AuditLogEntry): Promise<void> {
  for (const destination of auditConfig.destinations) {
    try {
      ...
      console.error(`Failed to write audit log to ${destination}:`, error);
    }
  }
}
...
async function writeToDatabase(entry: AuditLogEntry): Promise<void> {
  ...
  console.log(`[DB] Audit log saved: ${entry.id}`);
}
...
async function writeToSyslog(entry: AuditLogEntry): Promise<void> {
  ...
  console.log(`[SYSLOG] Audit log sent: ${entry.id}`);
}

async function writeToWebhook(entry: AuditLogEntry): Promise<void> {
  // Simular envío a webhook
  console.log(`[WEBHOOK] Audit log sent: ${entry.id}`);
}
