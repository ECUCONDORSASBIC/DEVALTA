...
const { getFirestoreInstance } = require('../src/lib/firebase-admin.ts');
const { firebaseConfig } = require('../src/lib/firebase-config.ts');
...
async function populateFirebase() {
  ...
  try {
    // Poblar usuarios
    await populateCollection(firebaseConfig.firestore.collections.users, sampleUsers);
    
    // Poblar citas
    await populateCollection(firebaseConfig.firestore.collections.appointments, sampleAppointments);
    
    // Poblar pagos
    await populateCollection(firebaseConfig.firestore.collections.payments, samplePayments);
    
    // Poblar alertas
    await populateCollection(firebaseConfig.firestore.collections.alerts, sampleAlerts);
    
    // Poblar logs del sistema
    await populateCollection(firebaseConfig.firestore.collections.systemLogs, sampleSystemLogs);
