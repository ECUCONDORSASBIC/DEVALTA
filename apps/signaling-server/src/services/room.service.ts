...
import { Room, Participant, User } from '@altamedica/types';
...
export class RoomService {
  ...
  async joinRoom(roomId: string, user: User, connectionInfo: any): Promise<Participant | null> {
    ...
    const existingParticipant = Array.from(room.participants.values()).find(p => p.userId === user.id);
  ...
  async leaveRoom(roomId: string, userId: string): Promise<boolean> {
    ...
    const activeParticipants = Array.from(room.participants.values()).filter(p => p.status === 'connected');
  ...
  async updateParticipantStatus(roomId: string, userId: string, status: 'waiting' | 'connected' | 'disconnected'): Promise<boolean> {
    const room = this.rooms.get(roomId);
    if (!room) {
      return false;
    }
  ...
  async getRoomStats(): Promise<{ totalRooms: number; activeRooms: number; totalParticipants: number; }> {
    const activeRooms = Array.from(this.rooms.values()).filter(r => r.status === 'active');
    const totalParticipants = activeRooms.reduce((sum, room) => sum + room.participants.size, 0);
    return {
