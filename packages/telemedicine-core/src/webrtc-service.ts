...
export class WebRTCService {
  ...
  static async createVideoCall(params: {
    ...
    return {
      ...
      direct_doctor_url: `${VIDEO_SERVER_BASE_URL}/video-call/${result.room_id}?user_id=${params.doctor_email}&user_type=doctor`,
      direct_patient_url: `${VIDEO_SERVER_BASE_URL}/video-call/${result.room_id}?user_id=${params.patient_email}&user_type=patient`
    };
  }
  ...
  static async checkServerHealth(): Promise<boolean> {
    try {
      ...
      console.error('WebRTC server health check failed:', error);
    }
    return false;
  }
   ...
   */
  static generateDirectVideoUrl(roomId: string, userEmail: string, userType: 'doctor' | 'patient'): string {
