...
import { useMutation, useQueryClient } from '@altamedica/hooks';
...
export function useOptimisticCreateAppointment() {
  ...
  return useMutation({
    mutationFn: async (appointmentData: Omit<OptimisticAppointment, 'id' | 'status'>) => {
    ...
    onMutate: async (newAppointment) => {
      ...
      queryClient.setQueryData(['appointments'], (old: any) => {
        if (!old) return { data: [optimisticAppointment] };
        return {
          ...old,
    ...
    onSuccess: (data, variables, context) => {
      queryClient.setQueryData(['appointments'], (old: any) => {
        ...
        return {
          ...old,
          data: old.data.map((apt: OptimisticAppointment) =>
            apt.id === context?.optimisticAppointment.id ? data : apt
          ),
        };
      });
...
export function useOptimisticCompleteAppointment() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, notes }) => {
      ...
      const updateFn = (apt: OptimisticAppointment) => ({
        ...apt,
        status: 'completed',
        notes,
        completedAt: new Date().toISOString(),
