...
export class TechnicalKnowledgeService {
      ...
      if (npmData.repository?.url) {
        ...
        if (repoMatch) {
          ...
          try {
            ...
            console.warn(`No se pudo obtener métricas de GitHub para ${packageName}`);
          }
        }
      }
  ...
  async checkVulnerabilities(packageName: string): Promise<VulnerabilityInfo[]> {
    try {
      if (!process.env.SNYK_API_KEY) {
        console.warn('SNYK_API_KEY no configurado, saltando verificación de vulnerabilidades');
        return [];
      }
  ...
  async searchMDNDocumentation(query: string): Promise<any[]> {
    try {
      ...
      console.error('Error buscando documentación MDN:', error);
      return [];
    }
  }
  ...
  async getBundleSize(packageName: string): Promise<any> {
    try {
      ...
      console.error('Error obteniendo tamaño del bundle:', error);
      return null;
    }
  }
