/**
 * @fileoverview Hook para notificaciones del marketplace médico
 * @module @altamedica/marketplace-hooks
 * @description Hook especializado para manejo de notificaciones en tiempo real del marketplace B2B
 */
...
/**
 * Hook para manejo de notificaciones del marketplace médico
 * Proporciona conexión WebSocket en tiempo real y gestión de estado de notificaciones
 */
export function useMarketplaceNotifications(): UseMarketplaceNotificationsReturn {
  ...
  const sendNotification = useCallback(async (notification: Partial<MarketplaceNotification>) => {
    if (!user?.token) return;
