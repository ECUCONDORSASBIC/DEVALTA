// Módulo ESM - Paquete compartido @altamedica/medical-cache
// No mezclar sintaxis CJS/ESM. Uso exclusivo como ESM.
// ------------------------------------------------------
...
class MedicalCacheManager {
  ...
  private initializeCache(type: string, config: MedicalCacheConfig) {
    const cache = new LRUCache<string, MedicalCacheEntry>({
      ...
      ttl: config.ttl,
      updateAgeOnGet: true,
      dispose: (value: MedicalCacheEntry, key: string) => {
  ...
  private calculateMemoryUsage(cache: any): number {
    ...
    if (entry) {
      ...
      if (entry.expiresAt > Date.now()) {
        ...
        const data = config.encryptSensitiveData && entry.encrypted 
          ? this.decryptData(entry.data as string) as T
          : entry.data as T
