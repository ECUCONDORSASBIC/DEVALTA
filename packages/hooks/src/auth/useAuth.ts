...
export function useAuth(options: UseAuthOptions = {}): UseAuthReturn {
  ...
  const scheduleTokenRefresh = useCallback((expiresAt: Date) => {
    ...
    const timeUntilRefresh = expiresAt.getTime() - Date.now() - config.tokens.AUTO_REFRESH_THRESHOLD;
  ...
  }, [config.tokens.AUTO_REFRESH_THRESHOLD]);
  ...
  const login = useCallback(async (credentials: LoginCredentials): Promise<void> => {
    ...
    try {
      ...
      // Procesar datos de usuario y tokens
      ...
      // Actualizar estado
      updateAuthState('authenticated', userData, tokenData);
      
      // Guardar en storage si está habilitado
      saveAuthData(userData, tokenData);

      // Programar renovación de tokens
  ...
  const register = useCallback(async (userData: RegisterData): Promise<void> => {
    ...
    try {
      ...
      if (data.accessToken) {
        ...
        updateAuthState('authenticated', newUser, tokenData);
        saveAuthData(newUser, tokenData);

        if (config.tokens.automaticRefresh) {
          scheduleTokenRefresh(tokenData.expiresAt);
        }
      } else {
  ...
  const initializeAuth = useCallback(async () => {
    ...
    try {
      ...
      // Intentar recuperar datos de storage
      const storedUser = storage.current.get<User>('user');
      const storedTokens = storage.current.get<AuthTokens>('tokens');
      ...
      if (now >= expiresAt) {
        ...
        try {
          ...
          console.warn('Failed to refresh expired tokens:', error);
          clearAuthData();
          setAuthState('unauthenticated');
          return;
        }
        ...
        // Tokens válidos
        updateAuthState('authenticated', storedUser, storedTokens);
  ...
  }, [refreshTokens, updateAuthState, clearAuthData, config.tokens.automaticRefresh, scheduleTokenRefresh]);
