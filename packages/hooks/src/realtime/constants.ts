...
import type { ReconnectionConfig, HeartbeatConfig } from './types';
...
export function isCriticalMedicalEvent(eventType: string): boolean {
  return MEDICAL_CONFIG.CRITICAL_EVENTS.includes(eventType);
}
...
export function getMessagePriority(priority: string): number {
  return MEDICAL_CONFIG.MESSAGE_PRIORITIES[priority.toUpperCase() as keyof typeof MEDICAL_CONFIG.MESSAGE_PRIORITIES] || 0;
}
