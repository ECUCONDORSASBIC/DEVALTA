...
interface UseTelemedicineReturn {
  ...
  updateSession: (sessionId: string, updates: Partial<TelemedicineSession>) => Promise<TelemedicineSession>;
}
...
export function useTelemedicine(options: UseTelemedicineOptions = {}): UseTelemedicineReturn {
  const { enabled = true, autoJoin = false } = options;
  ...
  const updateSessionMutation = useMutation({
    mutationFn: async ({ sessionId, updates }: { sessionId: string; updates: Partial<TelemedicineSession> }): Promise<TelemedicineSession> => {
  ...
  const updateSession = useCallback(
    async (sessionId: string, updates: Partial<TelemedicineSession>): Promise<TelemedicineSession> => {
...
export function useVideoCall(): UseVideoCallReturn {
  ...
  const startCall = useCallback(async (roomId: string, callConfig?: Partial<VideoCallConfig>) => {
    try {
