...
import type { 
  ...
  Report,
  Hypothesis 
} from '@altamedica/diagnostic-engine/dist/types';
...
export function useDiagnosticEngine(options: UseDiagnosticEngineOptions) {
  const engineRef = useRef<DiagnosticEngine | null>(null);
  ...
  const initializeEngine = useCallback(() => {
    if (!engineRef.current) {
      engineRef.current = new DiagnosticEngine();
    }
  ...
  const submitAnswer = useCallback(async (answer: Answer) => {
    ...
    try {
      ...
      const safetyWarnings = report?.safetyWarnings || [];
      ...
      const isComplete = !nextQuestion || 
        (options.maxQuestions && session.answeredQuestions.length >= options.maxQuestions - 1);
