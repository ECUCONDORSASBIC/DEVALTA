import { getFirebaseFirestore, serverTimestamp, addDoc, collection } from '../client';
...
export class MedicalAuditService {
  ...
  public async logMedicalAccess(
    ...
    try {
      ...
      const auditLog: MedicalAuditLog = {
        ...
        resourcePath: this.buildResourcePath(access.resourceType, access.resourceId, access.patientId),
      ...
      console.log(`🔍 Medical access logged: ${access.action} on ${access.resourceType} by ${user.role}`);
  ...
  private determineMedicalRecordType(resourceType: string): MedicalAuditLog['medicalRecordType'] | undefined {
    switch (resourceType) {
      case 'MEDICAL_RECORD': return 'DIAGNOSIS';
      case 'PRESCRIPTION': return 'PRESCRIPTION';
      case 'VITAL_SIGNS': return 'LAB_RESULT';
  ...
  private async sendSecurityAlert(auditLog: MedicalAuditLog): Promise<void> {
    try {
      ...
      console.log(`🚨 Security alert sent for ${auditLog.riskLevel} risk access`);
    } catch (error) {
  ...
  private async reportAuditFailure(error: any, context: any): Promise<void> {
    try {
      ...
      console.error('❌❌ CRITICAL: Could not report audit failure:', reportError);
