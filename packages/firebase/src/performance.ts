...
export class PerformanceMonitor {
   ...
   * Monitor API calls
   */
  monitorApiCall(endpoint: string, method: string = 'GET'): {
    start: () => void;
   ...
   */
  monitorFirebaseOperation(operation: string, collection?: string): {
    start: () => void;
  ...
  setCustomAttribute(attribute: string, value: string): void {
    this.traces.forEach((traceInstance, traceName) => {
      try {
        ...
        console.log(`📝 Custom attribute set for ${traceName}: ${attribute}=${value}`);
      } catch (error) {
        console.error(`Failed to set attribute for ${traceName}:`, error);
      }
    });
  }
}
...
export class PerformanceAnalytics {
  ...
  private static checkThresholds(metric: PerformanceMetrics): void {
    ...
    if (operation.startsWith('api_') && duration > PERFORMANCE_THRESHOLDS.API_RESPONSE_TIME_MS) {
      console.warn(`⚠️ PERFORMANCE ALERT: API ${operation} took ${duration}ms (threshold: ${PERFORMANCE_THRESHOLDS.API_RESPONSE_TIME_MS}ms)`);
    }

    // Page load time alert
    if (operation.startsWith('page_load_') && duration > PERFORMANCE_THRESHOLDS.PAGE_LOAD_TIME_MS) {
      console.warn(`⚠️ PERFORMANCE ALERT: Page ${operation} took ${duration}ms (threshold: ${PERFORMANCE_THRESHOLDS.PAGE_LOAD_TIME_MS}ms)`);
    }

    // Firebase query time alert
    if (operation.startsWith('firebase_') && duration > PERFORMANCE_THRESHOLDS.FIREBASE_QUERY_TIME_MS) {
      console.warn(`⚠️ PERFORMANCE ALERT: Firebase ${operation} took ${duration}ms (threshold: ${PERFORMANCE_THRESHOLDS.FIREBASE_QUERY_TIME_MS}ms)`);
    }
  }
