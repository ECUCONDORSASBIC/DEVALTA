...
function initializeFirebaseApp() {
  ...
  if (useEmulator) {
    ...
    try {
      ...
      console.log('⚠️ Auth emulator ya conectado o no disponible');
    }
    ...
    try {
      ...
      console.log('⚠️ Firestore emulator ya conectado o no disponible');
    }

    try {
      connectStorageEmulator(storage, 'localhost', 9199);
    } catch (error) {
      console.log('⚠️ Storage emulator ya conectado o no disponible');
    }
  } else {
